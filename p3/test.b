void testdecl(void a) { bool x; void x; int x; x = 1; }
void testfunc1(int x, int y){x=2; y =4;}
struct testStruct { int num1;}
struct testDoubleStruct { int val; bool next; struct testStruct a; }
int exp(){ void x; x = 2132; x = wergnojsa; x = True; x = False; x = -1; x = 2 + (x + 1); x = 2*(x+1); x = 1*-1; x = -1; ret x;}
void testinvoke() { func1(2,3); hash.next = 3;  }
